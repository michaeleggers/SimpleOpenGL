...
vec3 material_emissive_color = vec3(0.2f, 0.f, 0.5f);
vec3 material_ambient_color = vec3(0.f, 0.09f, 0.2f);
...
void main () {
    ...
    vec3 light_direction = normalize(g_light_pos - pos.xyz);
    ...
    frag_color = vec4 (emissive+diffuse+specular+specular_ex, 1.f);
}
